...
public class Util {
    ...
    /**
     * Converts a given integer to a binary byte array of the given length.
     * If the given length is smaller than the length of the binary
     * representation, the remaining bits will be lost.
     *
     * @param x the integer to convert
     * @param n the length of the byte array
     * @return a binary representation of the integer, expanded (or limited)
     * to the given number of bits.
     */
    public static byte[] indexToByteArray(int x, int n) {
        byte[] binary = new byte[n];
        for (int i = 0; i < n; i++) {
            binary[n - i - 1] = (byte) (x & 1);
            x >>= 1;
        }
        return binary;
    }
